...
fn timing_side_channel_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("constant_time_sphincs_verification", |b| {
        ...
        let valid_signature = sphincs_sign(&sk, message).unwrap();
    ...
    group.bench_function("constant_time_transition_verification", |b| {
        ...
        let device_info = DeviceInfo::new("timing_device", vec![1, 2, 3, 4]);
        ...
        let valid_op = Operation::Generic {
            operation_type: "valid_op".to_string(),
            data: vec![1, 2, 3, 4],
            message: String::new(),
        };

        let valid_state = state_machine.execute_transition(valid_op.clone()).unwrap();
    ...
    group.bench_function("constant_time_precommitment_validation", |b| {
        ...
        let op = Operation::Generic {
            operation_type: "precommit_op".to_string(),
            data: vec![9, 10, 11, 12],
            message: String::new(),
        };

        // Generate valid precommitment
        let (_, valid_positions) = state_machine.generate_precommitment(&op).unwrap();
    ...
    group.bench_function("constant_time_serialization", |b| {
        ...
        let mut state_1 = State::new_genesis(vec![1, 2, 3, 4], device_info.clone());
        let mut state_2 = State::new_genesis(vec![5, 6, 7, 8], device_info);

        // Ensure states have hash computed
        let hash_1 = state_1.compute_hash().unwrap();
        let hash_2 = state_2.compute_hash().unwrap();
        ...
        b.iter_batched_ref(
            ...
            |state| {
                // Serialize state and record result
                black_box(bincode::serialize(state))
            },
            criterion::BatchSize::SmallInput,
        );
    });
...
criterion_group! {
    name = benches;
    config = bench::configure_criterion("timing_analysis")();
