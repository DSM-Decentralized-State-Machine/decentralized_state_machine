...
impl RelationshipStatePair {
    ...
    pub fn get_last_synced_state(&self) -> Option<State> {
        // Check if we have a saved last synced state
        if let Some(synced_data) = self.verification_metadata.get("last_synced_state") {
            if !synced_data.is_empty() {
                // Try to deserialize the state
                if let Ok(state) = bincode::deserialize::<State>(synced_data) {
                    return Some(state);
                }
            }
        }
    ...
    /// This is used for storing unilateral transactions that need to be synced
    /// when both parties are online
    pub fn add_pending_transaction(&mut self, state: State) -> Result<(), DsmError> {
        // Ensure the state has a valid relationship context
    ...
    /// Returns states that have been added as pending but not yet synchronized
    pub fn get_pending_unilateral_transactions(&self) -> Vec<State> {
        if let Some(pending_data) = self.verification_metadata.get("pending_transactions") {
            if !pending_data.is_empty() {
                // Try to deserialize the pending transactions
                // This is a simplified implementation - in a production system,
                // we'd need to handle multiple states stored sequentially
                if let Ok(states) = bincode::deserialize::<Vec<State>>(pending_data) {
                    return states;
                } else {
                    // Try reading a single state if the vec deserialization fails
                    if let Ok(state) = bincode::deserialize::<State>(pending_data) {
                        return vec![state];
                    }
                }
            }
