...
mod tests {
    ...
    fn test_constants() {
        ...
        assert_eq!(PROTOCOL_MAGIC, [0x53, 0x45, 0x43, 0x49]);
    }

    #[test]
    fn test_constants_module() {
        assert_eq!(constants::DEFAULT_TIMEOUT_MS, 30000);
        assert_eq!(constants::RETRY_DELAY_MS, 5000);
        ...
        assert_eq!(constants::DEFAULT_KEY_DERIVATION_ITERATIONS, 100000);
        assert_eq!(constants::TEE_ENCLAVE_PATH, "./enclave.signed.so");
    }
    ...
    fn test_helpers_hex_to_bytes() {
        assert_eq!(helpers::hex_to_bytes("48656c6c6f"), Some(vec![72, 101, 108, 108, 111]));
        assert_eq!(helpers::hex_to_bytes("48656c6c6f7"), None);
    }

    #[test]
    fn test_helpers_bytes_to_hex() {
        assert_eq!(helpers::bytes_to_hex(&[72, 101, 108, 108, 111]), "48656c6c6f");
    }
}
