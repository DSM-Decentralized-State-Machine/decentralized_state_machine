    ...
    if !SPHINCS_INITIALIZED.load(Ordering::SeqCst) {
        ...
        let verify_sphincs = || -> Result<(), String> {
            let result = std::panic::catch_unwind(|| {
                ...
                // Verify key sizes
                if pk.as_bytes().len() != pqcrypto_sphincsplus::sphincssha2256fsimple::public_key_bytes() {
                ...
                if sk.as_bytes().len() != pqcrypto_sphincsplus::sphincssha2256fsimple::secret_key_bytes() {
                ...
                match verify_detached_signature(&signature, test_message, &pk) {
                    Ok(_) => {},
                    Err(_) => return Err("Signature verification failed during initialization".to_string()),
                }
                ...
                match verify_detached_signature(&signature, modified_message, &pk) {
                    Ok(_) => return Err("Signature verification succeeded with wrong message".to_string()),
        ...
        match verify_sphincs() {
            Ok(_) => {
                tracing::info!("SPHINCS+ signature subsystem successfully initialized and verified");
