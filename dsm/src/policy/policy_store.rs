...
use crate::policy::policy_types::{PolicyAnchor, PolicyFile, TokenPolicy};
use crate::policy::policy_verification::verify_policy_anchor;
...
impl PolicyStore {
    ...
    pub async fn store_policy(&self, policy: &PolicyFile) -> Result<PolicyAnchor, DsmError> {
        ...
        cache.insert(anchor.clone(), CacheEntry {
            policy: token_policy,
            added: Instant::now(),
    ...
    pub async fn verify_and_get_policy(
        ...
        if let Some(data) = policy_data {
            ...
            if !verify_policy_anchor(data, anchor)? {
                return Err(DsmError::validation(
                    "Policy data does not match anchor",
                    None::<std::convert::Infallible>,
                ));
            }
            ...
            let policy_file: PolicyFile = serde_json::from_slice(data).map_err(|e| {
                DsmError::serialization(format!("Failed to deserialize policy data: {}", e), Some(e))
            })?;
    ...
    pub fn evict_expired(&self) {
        ...
        let expired: Vec<PolicyAnchor> = cache.iter()
