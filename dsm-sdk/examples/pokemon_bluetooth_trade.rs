...
use dsm_sdk::sdk::pokemon_sdk::{Pokemon, PokemonParams, PokemonTrainer, PokemonType, TradeConditions};
use std::cmp::min;
use std::sync::Arc;
...
use blake3::Hasher;
use hex;
use dsm_sdk::sdk::protocol_metrics::ProtocolMetrics;
...
impl MockPokemonBluetoothSDK {
    ...
    async fn start_scanning(&mut self) -> Result<(), DsmError> {
        self.discovered_devices = MOCK_BLUETOOTH_DEVICES.lock().unwrap()
            .iter()
    ...
    async fn connect_to_trainer(&self, device_name: &str) -> Result<(), DsmError> {
        if self.discovered_devices.contains(&device_name.to_string()) {
            ...
            Err(DsmError::fatal(format!("Bluetooth device not found: {}", device_name)))
        }
    }
    ...
    async fn propose_trade(&self, device_name: &str, trade_conditions: TradeConditions) -> Result<String, DsmError> {
        if self.discovered_devices.contains(&device_name.to_string()) {
            let trade_id = generate_secure_trade_id(&trade_conditions);
            debug!("Trade proposed to device: {}", device_name);
            Ok(trade_id)
        } else {
            Err(DsmError::fatal(format!("Device not found: {}", device_name)))
        }
    }
    ...
    async fn execute_trade(&mut self, trade_id: &str) -> Result<(), DsmError> {
        ...
        if !executed_trades.insert(trade_id.to_string()) {
            ...
            if let Some(trainer) = &mut self.trainer {
                if let Some(updated) = devices.iter()
        ...
        let (red_state, blue_state) = devices.iter_mut()
            .filter(|t| {
        ...
        let (red_state, blue_state) = match (red_state, blue_state) {
            ...
            _ => return Err(DsmError::fatal("Could not find both trainers for trade execution"))
        };
        ...
        // This is a fixed mapping for demonstration; production would use lookup tables
        {
            ...
            if let (Some(charizard), Some(venusaur)) = (charizard, venusaur) {
                ...
                red_state.trainer.pokemon.insert("PKM_003".to_string(), venusaur);
                blue_state.trainer.pokemon.insert("PKM_001".to_string(), charizard);
    ...
    async fn respond_to_trade(&self, trade_id: &str, _accept: bool, _reason: Option<&str>) -> Result<(), DsmError> {
    ...
    fn get_trainer(&self) -> Result<&PokemonTrainer, DsmError> {
        self.trainer.as_ref().ok_or_else(|| DsmError::fatal("Trainer not initialized"))
    }

    // Simulate getting a mutable reference to the trainer
#[allow(dead_code)]
    fn get_trainer_mut(&mut self) -> Result<&mut PokemonTrainer, DsmError> {
        self.trainer.as_mut().ok_or_else(|| DsmError::fatal("Trainer not initialized"))
    }
}
...
async fn execute_trade_sequence(
    ...
    red_result.map_err(|e| JoinErrorWrapper(e))?.map_err(DsmError::from)?;
    blue_result.map_err(|e| JoinErrorWrapper(e))?.map_err(DsmError::from)?;
...
for i in 0..lines_bulba.len() {
    println!("{}", lines_bulba[i]);
}
...
for i in 0..lines_char.len() {
    println!("{}", lines_char[i]);
}
    ...
    println!("\x1b[1;36m╚═══════════════════════════════════════════════════════════════════════════╝\x1b[0m");
    println!("\x1b[1;36m║  RED TRAINER: {}                                                  ║\x1b[0m", red_trainer.device_info.device_id);
    println!("\x1b[1;36m║  BLUE TRAINER: {}                                                ║\x1b[0m", blue_trainer.device_info.device_id);
    println!("\x1b[1;36m║  TRADE CONDITIONS:                                                        ║\x1b[0m");
    println!("\x1b[1;36m║    Offered: {}                                                       ║\x1b[0m", trade_conditions.offered_pokemon_id);
    println!("\x1b[1;36m║    Requested: {}                                                     ║\x1b[0m", trade_conditions.requested_pokemon_id.unwrap_or_default());
    println!("\x1b[1;36m║    Sender: {}                                                    ║\x1b[0m", trade_conditions.sender_id);
    println!("\x1b[1;36m║    Recipient: {}                                                ║\x1b[0m", trade_conditions.recipient_id);
    println!("\x1b[1;36m╚═══════════════════════════════════════════════════════════════════════════╝\x1b[0m\n");
    ...
    println!("\x1b[1;37m║\x1b[0m \x1b[1;32mCrypto Operations\x1b[0m:   {:<45}{:>11}\x1b[1;37m║\x1b[0m", metrics.crypto_operations, "");
