...
impl SubscriptionManager {
    ...
    pub fn create_subscription(
        ...
        {
            let mut subscriptions = self.subscriptions.write().map_err(|_| StorageNodeError::Internal)?;
            subscriptions.insert(subscription_id.clone(), subscription.clone());
        }
        
        // Add to subscription history
        {
            let mut history = self.subscription_history.write().map_err(|_| StorageNodeError::Internal)?;
            let node_history = history.entry(request.node_id.clone()).or_insert_with(Vec::new);
        ...
        if let Some(vault_id) = &request.payment_vault_id {
            ...
            let time_proof = dsm::vault::FulfillmentProof::TimeProof {
                reference_state: reference_state.hash.clone(),
            ...
            match self.dlv_manager.try_unlock_vault(
                ...
                Ok(_) => {
                    ...
                    debug!("Payment vault {} exists for subscription {}", vault_id, subscription_id);
                },
                Err(e) => {
    ...
    pub fn verify_subscription_payment(
        ...
        let mut subscription = {
            let subscriptions = self.subscriptions.read().map_err(|_| StorageNodeError::Internal)?;
            
            match subscriptions.get(&request.subscription_id) {
                Some(sub) => sub.clone(),
                None => return Err(StorageNodeError::NotFound(format!(
                    "Subscription {} not found", request.subscription_id
                ))),
            }
        };
        ...
        let verifier_pubkey = &self.config.public_key;
        
        // Try to unlock and claim the payment vault
        match self.dlv_manager.try_unlock_vault(
            &subscription.payment_vault_id,
            time_proof.clone(),
            verifier_pubkey,
            reference_state,
        ) {
                    ...
                    Ok(content) => {
                        ...
                        if let Some(amount) = payment_data.get("amount").and_then(|v| v.as_u64()) {
                            if amount >= subscription.amount_paid {
                                ...
                                info!("Payment verified for subscription {}: {} {}", 
    ...
    pub fn is_within_quota(
        ...
        Ok(
            projected_storage <= storage_quota &&
    ...
    fn check_expired_subscriptions(&self) -> Result<()> {
        ...
        for sub in subscriptions.values_mut() {
            if sub.active && now > (sub.end_timestamp + self.config.grace_period) {
                ...
                info!("Subscription {} for node {} has expired", sub.id, sub.node_id);
                sub.active = false;
            }
        }
