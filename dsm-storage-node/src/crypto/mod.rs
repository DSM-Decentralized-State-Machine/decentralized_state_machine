...
use dsm::crypto::{
    decrypt_from_sender, encrypt_for_recipient, generate_keypair,
    sign_data, verify_signature, sphincs::generate_sphincs_keypair,
    sphincs::sphincs_sign, sphincs::sphincs_verify
};
use pqcrypto_mlkem::keypair as kyber_keypair;
use pqcrypto_sphincsplus::keypair as sphincs_keypair;
...
pub fn encrypt_data(recipient_public_key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    ...
    let encrypted = encrypt_for_recipient(recipient_public_key, data)
        .map_err(|e| StorageNodeError::Encryption(format!("Failed to encrypt data: {}", e)))?;
...
pub fn decrypt_data(sender_public_key: &[u8], private_key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    ...
    let decrypted = decrypt_from_sender(sender_public_key, private_key, data)
        .map_err(|e| StorageNodeError::Encryption(format!("Failed to decrypt data: {}", e)))?;
