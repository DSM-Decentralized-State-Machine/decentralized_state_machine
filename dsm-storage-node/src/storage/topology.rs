...
impl NodeId {
    ...
    pub fn from_string(s: &str) -> Result<Self, crate::error::StorageNodeError> {
        if s.len() != 64 {
            return Err(crate::error::StorageNodeError::InvalidNodeId(
                format!("Invalid NodeId length: {}", s.len())
            ));
        }
        
        let bytes = match hex::decode(s) {
            Ok(b) => b,
            Err(e) => return Err(crate::error::StorageNodeError::InvalidNodeId(
                format!("Invalid NodeId hex: {}", e)
            )),
        };
        
        if bytes.len() != 32 {
            return Err(crate::error::StorageNodeError::InvalidNodeId(
                format!("Decoded length is not 32 bytes: {}", bytes.len())
            ));
        }
    ...
    pub fn xor_distance(&self, other: &NodeId) -> Distance {
        ...
        result.iter_mut().zip(self.0.iter().zip(other.0.iter()))
...
pub struct HybridTopology {
    ...
    #[allow(dead_code)]
    network_client: Option<Arc<dyn NetworkClient + Send + Sync>>,
    /// Metrics collector
    #[allow(dead_code)]
...
impl HybridTopology {
    ...
    pub fn add_node(&mut self, node_id: NodeId, address: SocketAddr, region: Option<u8>, reputation: u8) -> Result<(), StorageNodeError> {
        ...
        match connection_type {
            ...
            ConnectionType::LongRange => {
                ...
                if let Some(pos) = self.long_range_connections.iter().position(|n| n.node_id == node_id) {
                ...
                if self.long_range_connections.len() > self.config.long_range_connection_count {
                    self.long_range_connections.sort_by_key(|n| std::cmp::Reverse(n.last_seen));
                    self.long_range_connections.truncate(self.config.long_range_connection_count);
                }
            },
            ...
            ConnectionType::Reputation => {
                ...
                if reputation >= self.config.min_reputation_threshold {
                    // Remove if already exists
                    if let Some(pos) = self.reputation_connections.iter().position(|n| n.node_id == node_id) {
                    ...
                    self.reputation_connections.sort_by_key(|n| std::cmp::Reverse(n.reputation));
    ...
    fn determine_connection_type(&self, node_id: &NodeId, region: Option<u8>) -> ConnectionType {
        ...
        if let (Some(local_region), Some(node_region)) = (self.local_region, region) {
            if local_region != node_region {
                let region_connections = self.geographic_connections.get(&node_region).unwrap_or(&Vec::new()).to_vec();
                if region_connections.len() < self.config.geographic_connections_per_region {
    ...
    pub fn propagate_message(&self, message_id: [u8; 32], data: Vec<u8>, ttl: u8) -> Result<(), StorageNodeError> {
    ...
    pub fn refresh_if_needed(&mut self) -> Result<bool, StorageNodeError> {
        ...
        self.long_range_connections.retain(|node| node.last_seen > stale_threshold);
        ...
        self.reputation_connections.retain(|node| node.last_seen > stale_threshold);
        
        // Clean all_nodes map
        self.all_nodes.retain(|_, node| node.last_seen > stale_threshold);
        ...
        if let Some(network_client) = &self.network_client {
            ...
            if self.local_region.is_some() {
                ...
                for region in 0..8u8 { // Assume 8 possible regions
                    if Some(region) != self.local_region {
                        let count = self.geographic_connections.get(&region).map_or(0, |v| v.len());
        ...
        if let Some(metrics) = &self.metrics_collector {
            ...
            metrics.record_custom_metric("geographic_diversity", self.geographic_connections.len() as f64);
        }
    ...
    pub fn update_node_reputation(&mut self, node_id: &NodeId, success: bool, response_time_ms: Option<u64>) {
        if let Some(node) = self.all_nodes.get_mut(node_id) {
            ...
            if success {
                ...
                if let Some(response_time) = response_time_ms {
                    ...
                    node.metrics.avg_response_time = node.metrics.avg_response_time * 0.9 + (response_time as f64) * 0.1;
                ...
                match old_type {
                    ...
                    ConnectionType::Geographic => {
                        if let Some(region) = node.region {
                            if let Some(region_connections) = self.geographic_connections.get_mut(&region) {
                                if let Some(pos) = region_connections.iter().position(|n| n.node_id == *node_id) {
                                    region_connections.remove(pos);
                                }
                            }
    ...
    pub fn get_node_connections(&self) -> Vec<(NodeId, Vec<NodeId>)> {
        ...
        let my_connections: Vec<NodeId> = self.all_neighbors()
            .into_iter()
