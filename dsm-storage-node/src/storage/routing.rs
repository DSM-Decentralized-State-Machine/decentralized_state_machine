...
impl RoutingTable {
    ...
    fn calculate_route(&self, target: &NodeId, max_hops: usize) -> Option<Vec<StorageNode>> {
        ...
        while let Some(current_id) = queue.pop_front() {
            ...
            for neighbor in neighbors {
                ...
                // Skip failed routes
                if self.failed_routes.contains_key(&(current_id.clone(), neighbor_id.clone())) {
                    continue;
                }
    ...
    fn get_node_neighbors(&self, node: &StorageNode, topology: &SmallWorldTopology) -> Vec<StorageNode> {
        ...
        potential_neighbors.extend(
            topology.find_closest_nodes(&node_id, 5, None)
        );
    ...
    pub fn mark_route_failed(&self, from: &NodeId, to: &NodeId) {
        self.failed_routes.insert((from.clone(), to.clone()), Instant::now());
    ...
    fn greedy_routing(&self, target: &NodeId, topology: &SmallWorldTopology) -> Option<StorageNode> {
    ...
    fn perimeter_routing(&self, target: &NodeId, topology: &SmallWorldTopology) -> Option<StorageNode> {
        ...
        closest.retain(|node| {
            ...
            node_dist < target_dist && !self.failed_routes.contains_key(&(self.self_id.clone(), node_id))
        });
    ...
    fn probabilistic_routing(&self, target: &NodeId, topology: &SmallWorldTopology) -> Option<StorageNode> {
    ...
    fn hybrid_routing(&self, target: &NodeId, topology: &SmallWorldTopology) -> Option<StorageNode> {
        // Try greedy first
        ...
        if let Some(node) = &greedy_result {
            ...
            if self.failed_routes.contains_key(&(self.self_id.clone(), node_id)) {
    ...
    pub fn find_responsible_nodes_for_id(&self, blinded_id: &str, count: usize) -> Vec<StorageNode> {
...
impl EpidemicRouter {
    ...
    /// Find responsible nodes for a blinded ID
    pub fn find_responsible_nodes_for_id(&self, blinded_id: &str, count: usize) -> Vec<StorageNode> {
        self.routing_table.find_responsible_nodes_for_id(blinded_id, count)
    }
