...
impl OperationContext {
    ...
    pub fn new(collector: Arc<MetricsCollector>, operation_type: OperationType) -> Self {
        Self {
            collector,
            operation_type,
            ...
            key_id: None,
            data_size: None,
            is_read: operation_type == OperationType::Retrieve
    ...
    pub fn complete(self, outcome: OperationOutcome) {
        ...
        self.collector
            .record_operation(self.operation_type, outcome, latency_us, self.data_size);
...
impl MetricsCollector {
    ...
    pub fn new(node_id: String, config: MetricsCollectorConfig) -> Self {
        ...
        collector.operation_metrics.insert(
            OperationType::Store,
            OperationMetrics::new(OperationType::Store),
        );
        collector.operation_metrics.insert(
            OperationType::Retrieve,
            OperationMetrics::new(OperationType::Retrieve),
        );
        collector.operation_metrics.insert(
            OperationType::Delete,
            OperationMetrics::new(OperationType::Delete),
        );
        collector.operation_metrics.insert(
            OperationType::Exists,
            OperationMetrics::new(OperationType::Exists),
        );
        collector.operation_metrics.insert(
            OperationType::List,
            OperationMetrics::new(OperationType::List),
        );
        collector.operation_metrics.insert(
            OperationType::GossipSend,
            OperationMetrics::new(OperationType::GossipSend),
        );
        collector.operation_metrics.insert(
            OperationType::GossipReceive,
            OperationMetrics::new(OperationType::GossipReceive),
        );
        collector.operation_metrics.insert(
            OperationType::AntiEntropy,
            OperationMetrics::new(OperationType::AntiEntropy),
        );
        collector.operation_metrics.insert(
            OperationType::TopologyUpdate,
            OperationMetrics::new(OperationType::TopologyUpdate),
        );
        collector.operation_metrics.insert(
            OperationType::HealthCheckPing,
            OperationMetrics::new(OperationType::HealthCheckPing),
        );
        collector.operation_metrics.insert(
            OperationType::HealthCheckPong,
            OperationMetrics::new(OperationType::HealthCheckPong),
        );
    ...
    pub fn start(self: &Arc<Self>) {
        ...
        tokio::spawn(async move {
            ...
            let mut collection_interval = tokio::time::interval(Duration::from_millis(
                collector.config.collection_interval_ms,
            ));

            let mut snapshot_interval =
                tokio::time::interval(Duration::from_millis(collector.config.snapshot_interval_ms));
    ...
    fn update_metrics(&self) {
        ...
        // Update epidemic metrics
        {
            ...
            if epidemic_metrics.gossip_rounds > 0 {
                epidemic_metrics.avg_gossip_fanout =
                    (epidemic_metrics.avg_gossip_fanout * 0.95) + (gossip_fanout as f64 * 0.05);
                epidemic_metrics.avg_entries_per_gossip = (epidemic_metrics.avg_entries_per_gossip
                    * 0.95)
    ...
    fn prune_key_metrics(&self) {
        ...
        let mut metrics_with_timestamps: Vec<(String, u64)> = self
            .key_metrics
            .iter()
            .map(|entry| {
        ...
        let to_remove = metrics_with_timestamps
            .into_iter()
            .skip(to_keep)
            .map(|(k, _)| k)
            .collect::<Vec<_>>();
    ...
    fn prune_node_metrics(&self) {
        ...
        let mut metrics_with_timestamps: Vec<(String, u64)> = self
            .node_metrics
            .iter()
            .map(|entry| (entry.key().clone(), entry.value().last_updated))
            .collect();
    ...
    fn get_top_node_metrics(&self, count: usize) -> Vec<NodeMetrics> {
        let mut metrics: Vec<NodeMetrics> = self
            .node_metrics
            .iter()
    ...
    fn get_top_key_metrics(&self, count: usize) -> Vec<KeyMetrics> {
        let mut metrics: Vec<KeyMetrics> = self
            .key_metrics
            .iter()
            .map(|entry| entry.value().clone())
            .collect();
    ...
    pub fn record_node_operation(
        ...
        // Update latencies
        entry.recent_average_latency_us =
            (entry.recent_average_latency_us * 0.9) + (latency_us as f64 * 0.1);
        ...
        if entry.average_latency_us == 0.0 {
            ...
            entry.average_latency_us =
                (entry.average_latency_us * 0.99) + (latency_us as f64 * 0.01);
        }
    ...
    pub fn record_gossip_round(&self, fanout: u64, entries: u64) {
        ...
        self.current_entries_per_gossip
    ...
    pub fn record_cross_region_propagation(
        &self,
        from_region: &str,
        to_region: &str,
        time_ms: u64,
...
mod tests {
    ...
    fn test_latency_histogram() {
        ...
        // Check average
        assert_eq!(
            histogram.average_latency_us(),
            (100 + 500 + 5000) as f64 / 3.0
        );
