...
impl SmallWorldTopology {
    ...
    pub fn new(self_node: StorageNode, config: SmallWorldConfig) -> Self {
        ...
        topology.known_nodes.insert(NodeId::from(&self_node), self_node);
    ...
    pub fn update_long_links(&mut self) {
        ...
        if self.long_links.len() < self.max_long_links {
            let mut candidates: Vec<&StorageNode> = self.known_nodes.values()
    ...
    pub fn find_closest_nodes(&self, id: &NodeId, count: usize, exclude: Option<&HashSet<NodeId>>) -> Vec<StorageNode> {
        // First, find the closest bucket
        let closest: Vec<StorageNode> = self.known_nodes.iter()
            .filter(|(node_id, _)| {
    ...
    pub fn find_closest_node(&self, id: &NodeId, skip: usize, exclude: Option<&HashSet<NodeId>>) -> Option<StorageNode> {
    ...
    pub fn get_epidemic_targets(&self, key_hash: &[u8; 32], propagation_factor: usize) -> Vec<StorageNode> {
        ...
        if immediate_count > 0 {
            targets.extend(self.immediate_neighbors.iter().take(immediate_count).cloned());
        }
