...
use super::bluetooth_transport::{BluetoothMessage, BluetoothMode, BluetoothTransport};
...
impl PokemonBluetoothSDK {
    ...
    pub fn stop_scanning(&self) -> Result<(), DsmError> {
        self.bluetooth.stop_scanning().map_err(DsmError::from)
    }
    ...
    pub fn stop_advertising(&self) -> Result<(), DsmError> {
        self.bluetooth.stop_advertising().map_err(DsmError::from)
    }
    ...
    pub async fn propose_trade(
        ...
        let serialized_vault = bincode::serialize(&trade_vault).map_err(|e| {
            DsmError::serialization("Failed to serialize trade vault for signing", Some(e))
        })?;
        ...
        // This establishes cryptographic intent before the trade is fully executed
        {
            ...
            let _next_state =
                state_machine.apply_operation(current_state, pre_commit_op, next_entropy)?;

            tracing::debug!(
                "Pre-commitment for trade {} recorded in hash chain",
                trade_vault.trade_id
            );
        }
    ...
    pub async fn execute_trade(&self, trade_id: &str) -> Result<(), DsmError> {
        ...
        {
            let sessions = self.trade_sessions.lock().unwrap();
            let session_ref = sessions.get(trade_id).ok_or_else(|| {
                DsmError::validation("Trade session not found", None::<std::convert::Infallible>)
            })?;
            ...
            if let Some(proposer_sig) = &proposer_sig_clone {
                ...
                let verification_bytes = bincode::serialize(&vault_copy).map_err(|e| {
                    DsmError::serialization(
                        "Failed to serialize vault for signature verification",
                        Some(e),
                    )
                })?;

                // Get relationship context for the other party
                if let Some(_context) = self
                    .pokemon_sdk
                    .identity_sdk
                    .get_relationship_context(&vault_copy.conditions.sender_id)
                {
                    // Verify signature
            ...
            if let Some(recipient_sig) = &recipient_sig_clone {
                ...
                let verification_bytes = bincode::serialize(&vault_copy).map_err(|e| {
                    DsmError::serialization(
                        "Failed to serialize vault for recipient signature verification",
                        Some(e),
                    )
                })?;
                ...
                if let Some(context) = self
                    .pokemon_sdk
                    .identity_sdk
                    .get_relationship_context(&vault_copy.conditions.recipient_id)
                {
                    // Verify signature
                    signature_verifications += 1;
                    let valid = dsm::crypto::signatures::SignatureKeyPair::verify_raw(
        ...
        let pokemon_to_transfer = trainer.get_pokemon(&offered_pokemon_id).ok_or_else(|| {
            DsmError::validation(
        ...
        self.bluetooth
            .send_pokemon(&remote_device_id, &offered_pokemon_id, serialized_pokemon)
            .await
            .map_err(DsmError::from)?;
        ...
        tracing::info!(
            ...
            security_level = format!(
                "Cryptographic Identity Verification with {} signatures",
        ...
        for device in devices {
            ...
            tokio::spawn(async move {
                ...
                while let Some(message) = stream.next().await {
                    match message {
                        BluetoothMessage::ConnectionRequest {
                            device_id: _,
                            device_name: _,
                        } => {
                        ...
                        BluetoothMessage::ConnectionResponse {
                            accepted,
                            error_message,
                        } => {
                        ...
                        BluetoothMessage::TradeRequest {
                            ...
                            if let Ok(trade_vault) = TradeVault::deserialize(&serialized_trade) {
                                ...
                                if let Some(proposer_sig) = &trade_vault.proposer_signature {
                                    ...
                                    if let Some(sender_context) = pokemon_sdk_clone
                                        .identity_sdk
                                        .get_relationship_context(&trade_vault.conditions.sender_id)
                                    {
                                        ...
                                        // Serialize for verification
                                        if let Ok(verification_bytes) =
                                            bincode::serialize(&verification_vault)
                                        {
                                            // Verify signature
                                            ...
                                            if let Err(e) = verification_result {
                                                println!(
                                                    "Error verifying trade proposal signature: {}",
                                                    e
                                                );
                                                continue;
                                            ...
                                            println!(
                                                "Trade proposal signature verified successfully"
                                            );
                                        }
                                    }
                                }
                        ...
                        BluetoothMessage::TradeResponse {
                            trade_id,
                            accepted,
                            counter_offer,
                        } => {
                            ...
                            if let Some(session) = sessions.get_mut(&trade_id) {
                                ...
                                if accepted {
                                    ...
                                    let recipient_context = pokemon_sdk_clone
                                        .identity_sdk
                                    ...
                                    if let Some(_context) = recipient_context {
                                        ...
                                        let serialized_vault =
                                            match bincode::serialize(&session.trade_vault) {
                                                Ok(data) => data,
                                                Err(e) => {
                                                    println!(
                                                        "Error serializing trade vault: {}",
                                                        e
                                                    );
                                                    continue;
                                                }
                                        ...
                                        println!(
                                            "No relationship context found for recipient {}",
                                            recipient_id
                                        );
                                        continue;
                                    }
                                    ...
                                    if let Some(counter_data) = counter_offer {
                                        if let Ok(_counter_vault) =
                                            TradeVault::deserialize(&counter_data)
                                        {
                                            println!(
                                                "Counter offer received for trade: {}",
                                                trade_id
                                            );
                        ...
                        BluetoothMessage::PokemonTransfer {
                            pokemon_id,
                            serialized_pokemon,
                        } => {
                            ...
                            // Deserialize the Pokemon
                            let pokemon_result =
                                bincode::deserialize::<Pokemon>(&serialized_pokemon);
                            ...
                            if let Some(current_state) = current_state_result {
                                ...
                                let next_entropy =
                                    derive_next_entropy(&current_state.entropy, &pokemon.hash);
                                ...
                                match state_machine.apply_operation(
                                    ...
                                    Ok(_) => {
                                        println!(
                                            "Pokemon receipt recorded in hash chain: {}",
                                            pokemon.id
                                        );
                                    }
                                    Err(e) => {
                                        println!(
                                            "Error recording Pokemon receipt in hash chain: {}",
                                            e
                                        );
                                    }
                                }
    ...
    pub fn record_pokemon_receipt(&self, pokemon: &Pokemon) -> Result<(), DsmError> {
        ...
        let serialized_pokemon = bincode::serialize(pokemon).map_err(|e| {
            DsmError::serialization("Failed to serialize Pokemon for hash chain", Some(e))
        })?;
        ...
        // Apply the operation to record the receipt
        let _next_state =
            state_machine.apply_operation(current_state, receipt_operation, next_entropy)?;
    ...
    fn bind_trade_to_hash_chain(&self, trade_vault: &TradeVault) -> Result<(), DsmError> {
        ...
        let serialized_vault = bincode::serialize(trade_vault).map_err(|e| {
            DsmError::serialization("Failed to serialize trade vault for hash chain", Some(e))
        })?;
        ...
        let _next_state =
            state_machine.apply_operation(current_state, bind_operation, next_entropy)?;
