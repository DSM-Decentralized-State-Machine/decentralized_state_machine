...
pub fn load_or_generate_keypair(
    ...
    if private_key_path.exists() && public_key_path.exists() {
        ...
        let private_key = fs::read(private_key_path).map_err(|e| {
            StorageNodeError::Encryption(format!("Failed to read private key: {}", e))
        })?;

        let public_key = fs::read(public_key_path).map_err(|e| {
            StorageNodeError::Encryption(format!("Failed to read public key: {}", e))
        })?;
        ...
        if let Some(parent) = private_key_path.parent() {
            if !parent.exists() {
                fs::create_dir_all(parent).map_err(|e| {
                    StorageNodeError::Encryption(format!("Failed to create key directory: {}", e))
                })?;
            }
        }
        ...
        // Save keys to files
        fs::write(private_key_path, &private_key).map_err(|e| {
            StorageNodeError::Encryption(format!("Failed to write private key: {}", e))
        })?;

        fs::write(public_key_path, &public_key).map_err(|e| {
            StorageNodeError::Encryption(format!("Failed to write public key: {}", e))
        })?;
