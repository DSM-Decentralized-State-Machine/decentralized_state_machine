...
use prometheus::{
    Registry, Histogram, IntCounter, IntGauge,
...
impl Metrics {
    ...
    pub fn new() -> Self {
        ...
        registry.register(Box::new(ips_collected.clone())).unwrap_or_else(|e| {
            error!("Failed to register ips_collected metric: {}", e);
        });
        
        registry.register(Box::new(legitimate_ips.clone())).unwrap_or_else(|e| {
            error!("Failed to register legitimate_ips metric: {}", e);
        });
        
        registry.register(Box::new(flagged_ips.clone())).unwrap_or_else(|e| {
            error!("Failed to register flagged_ips metric: {}", e);
        });
        
        registry.register(Box::new(collection_active.clone())).unwrap_or_else(|e| {
            error!("Failed to register collection_active metric: {}", e);
        });
        
        registry.register(Box::new(snapshots_created.clone())).unwrap_or_else(|e| {
            error!("Failed to register snapshots_created metric: {}", e);
        });
        
        registry.register(Box::new(country_count.clone())).unwrap_or_else(|e| {
            error!("Failed to register country_count metric: {}", e);
        });
        
        registry.register(Box::new(rate_limited_requests.clone())).unwrap_or_else(|e| {
            error!("Failed to register rate_limited_requests metric: {}", e);
        });
        
        registry.register(Box::new(collection_latency_seconds.clone())).unwrap_or_else(|e| {
            error!("Failed to register collection_latency_seconds metric: {}", e);
        });
        
        registry.register(Box::new(http_requests_total.clone())).unwrap_or_else(|e| {
            error!("Failed to register http_requests_total metric: {}", e);
        });
        
        registry.register(Box::new(http_errors_total.clone())).unwrap_or_else(|e| {
            error!("Failed to register http_errors_total metric: {}", e);
        });
        
        registry.register(Box::new(last_collection_timestamp.clone())).unwrap_or_else(|e| {
            error!("Failed to register last_collection_timestamp metric: {}", e);
        });
        
        registry.register(Box::new(last_snapshot_timestamp.clone())).unwrap_or_else(|e| {
        ...
        F: FnOnce() -> R,
    {
        ...
        let result = f();
        self.collection_latency_seconds.observe(timer.elapsed().as_secs_f64());
        result
    }
