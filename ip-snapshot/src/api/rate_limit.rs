...
impl GlobalRateLimiters {
    ...
    pub fn get_limiter(
        ...
        let limiters = match limit_type {
            ...
            RateLimitType::UserOperations => todo!(),
        };
        ...
        limiters
            ...
            .or_insert_with(|| {
                ...
                let quota = match limit_type {
                    ...
                    RateLimitType::UserOperations => (120, Duration::from_secs(60)), // 120 per minute
                };

                Arc::new(Mutex::new(DirectRateLimiter::<GCRA>::new(
                    std::num::NonZero::new(quota.0).unwrap(),
                    quota.1,
                )))
            })
            .clone()
    ...
    pub async fn check_limit(&self, ip: IpAddr, limit_type: RateLimitType) -> bool {
        // Get the limiter
        ...
        match limiter_guard.check() {
            Ok(_) => {
                // Not rate limited
                debug!(
                    "Rate limit check passed for IP {} (type: {:?})",
                    ip, limit_type
                );
                true
            }
...
#[allow(dead_code)]
pub async fn check_rate_limit(ip: IpAddr, limit_type: RateLimitType) -> Result<(), String> {
    // Check rate limit
    ...
    if limiters.check_limit(ip, limit_type).await {
        ...
        Err(format!(
            "Rate limit exceeded for IP {} (type: {:?})",
            ip, limit_type
        ))
    }
}
