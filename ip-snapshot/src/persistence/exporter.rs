...
pub async fn export_json(store: &SnapshotStore, output_path: &Path) -> Result<()> {
    ...
    let json_str = serde_json::to_string_pretty(&export_data).map_err(|e| {
        SnapshotError::Export(format!("Failed to serialize export data: {}", e))
    })?;
    
    fs::write(output_path, json_str).await.map_err(|e| {
        SnapshotError::Export(format!("Failed to write export file: {}", e))
    })?;
    
    info!("Export completed successfully. Wrote {} snapshots.", snapshot_array.len());
...
pub async fn export_csv(store: &SnapshotStore, output_path: &Path) -> Result<()> {
    ...
    let file = tokio::fs::File::create(output_path).await.map_err(|e| {
        SnapshotError::Export(format!("Failed to create CSV file: {}", e))
    })?;
    ...
    writer.write_record(&[
        ...
        SnapshotError::from(ExportError::Csv(e))
    })?;
    ...
    for metadata in snapshots {
        ...
        for entry in &entries {
            ...
            let record = vec![
                ...
                entry.geo.as_ref().and_then(|g| g.country_code.clone()).unwrap_or_default(),
                entry.geo.as_ref().and_then(|g| g.city.clone()).unwrap_or_default(),
                entry.geo.as_ref().and_then(|g| g.coordinates.map(|(lat, _)| lat.to_string())).unwrap_or_default(),
                entry.geo.as_ref().and_then(|g| g.coordinates.map(|(_, lon)| lon.to_string())).unwrap_or_default(),
                entry.network.asn.map(|asn| asn.to_string()).unwrap_or_default(),
            ...
            writer.write_record(&record).map_err(|e| {
                SnapshotError::Export(format!("CSV export error: {}", e))
            })?;
    ...
    info!("Export completed successfully. Wrote {} entries across {} snapshots.", 
...
pub async fn export_hash(store: &SnapshotStore, output_path: &Path) -> Result<()> {
    ...
    // Write to file
    let json_str = serde_json::to_string_pretty(&hash_data).map_err(|e| {
        SnapshotError::Export(format!("Failed to serialize hash data: {}", e))
    })?;
    
    fs::write(output_path, json_str).await.map_err(|e| {
        SnapshotError::Export(format!("Failed to write hash file: {}", e))
    })?;
    
    info!("Hash export completed successfully for {} snapshots. Master hash: {}", 
...
pub async fn export_analysis(store: &SnapshotStore, output_path: &Path) -> Result<()> {
    info!("Generating comprehensive snapshot analysis: {:?}", output_path);
    ...
    if snapshots.len() > 1 {
        ...
        for (_, snapshot_list) in &ip_tracking {
    ...
    // Write to file
    let json_str = serde_json::to_string_pretty(&analysis_data).map_err(|e| {
        SnapshotError::Export(format!("Failed to serialize analysis data: {}", e))
    })?;
    
    fs::write(output_path, json_str).await.map_err(|e| {
        SnapshotError::Export(format!("Failed to write analysis file: {}", e))
    })?;
    
    info!("Analysis completed successfully. Analyzed {} IPs across {} snapshots.", 
          total_ips, snapshots.len());
...
pub async fn export_dsm_compatible(
    ...
    dsm_entries.sort_by(|a, b| {
        a["ip"].as_str().unwrap_or_default().cmp(b["ip"].as_str().unwrap_or_default())
    });
    ...
    // Write to file
    let json_str = serde_json::to_string_pretty(&dsm_data).map_err(|e| {
        SnapshotError::Export(format!("Failed to serialize DSM data: {}", e))
    })?;
    
    fs::write(output_path, json_str).await.map_err(|e| {
        SnapshotError::Export(format!("Failed to write DSM file: {}", e))
    })?;
    
    info!("DSM-compatible export completed successfully for snapshot {}. DSM ID: {}", 
          snapshot_id, dsm_id);
...
pub async fn export_attestation(store: &SnapshotStore, output_path: &Path) -> Result<()> {
    ...
    for hash_obj in snapshot_hashes.iter() {
        ...
        let hash_bytes = hex::decode(hash_hex).map_err(|e| {
            SnapshotError::Export(format!("Failed to decode hash: {}", e))
        })?;
    ...
    // Write to file
    let json_str = serde_json::to_string_pretty(&attestation).map_err(|e| {
        SnapshotError::Export(format!("Failed to serialize attestation: {}", e))
    })?;
    
    fs::write(output_path, json_str).await.map_err(|e| {
        SnapshotError::Export(format!("Failed to write attestation file: {}", e))
    })?;
    
    info!("Attestation generated successfully. Commitment: {}", hex::encode(merkle_root));
