...
pub async fn verify_snapshot<P: AsRef<Path>>(snapshot_path: P) -> Result<VerificationResult> {
    ...
    let content = fs::read_to_string(path).await.map_err(|e| {
        SnapshotError::Validation(format!("Failed to read snapshot file: {}", e))
    })?;
    
    // Parse as JSON
    let data: Value = serde_json::from_str(&content).map_err(|e| {
        SnapshotError::Validation(format!("Failed to parse snapshot JSON: {}", e))
    })?;
...
async fn verify_raw_snapshot(data: &Value) -> Result<VerificationResult> {
    ...
    let entries: Vec<IpEntry> = serde_json::from_value(entries.clone()).map_err(|e| {
        SnapshotError::Validation(format!("Failed to parse entries: {}", e))
    })?;
    ...
    let timestamp = metadata.get("start_time")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse::<chrono::DateTime<Utc>>().ok())
        .unwrap_or_default();
    
    let data_hash = metadata.get("data_hash")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    let error_msg = if !is_valid {
        if !all_entries_valid {
            Some(format!("{} entries failed integrity check", invalid_entries))
        } else if !hash_valid {
            Some(format!("Data hash mismatch. Expected: {}, got: {}", data_hash, calculated_hash))
        } else {
...
async fn verify_export_snapshot(data: &Value) -> Result<VerificationResult> {
    // Parse export data
    let export_timestamp = data.get("export_timestamp")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse::<chrono::DateTime<Utc>>().ok())
        .unwrap_or_default();
    ...
    // Verify export hash
    let export_hash = data.get("export_hash")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    for snapshot in snapshots_array {
        ip_count += snapshot.get("entries_count")
            .and_then(|v| v.as_u64())
        ...
        if let Some(metadata) = snapshot.get("metadata") {
            country_count = country_count.max(
                metadata.get("country_count")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0) as usize
            );
            
            flagged_ips += metadata.get("flagged_ip_count")
                .and_then(|v| v.as_u64())
                .unwrap_or(0) as usize;
        }
    }
    ...
    let error_msg = if !hash_valid {
        Some(format!("Export hash mismatch. Expected: {}, got: {}", export_hash, calculated_hash))
    } else {
        None
    };
...
async fn verify_hash_snapshot(data: &Value) -> Result<VerificationResult> {
    ...
    // Verify master hash
    let master_hash = data.get("master_hash")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    for snapshot in snapshots_array {
        ip_count += snapshot.get("entry_count")
            .and_then(|v| v.as_u64())
            .unwrap_or(0) as usize;
    }
    
    let error_msg = if !hash_valid {
        Some(format!("Master hash mismatch. Expected: {}, got: {}", master_hash, calculated_master_hash))
    } else {
        None
    };
...
async fn verify_analysis_snapshot(data: &Value) -> Result<VerificationResult> {
    // Parse analysis data
    let analysis_timestamp = data.get("analysis_timestamp")
        .and_then(|v| v.as_str())
        .and_then(|s| s.parse::<chrono::DateTime<Utc>>().ok())
        .unwrap_or_default();
    
    // Verify verification hash
    let verification_hash = data.get("verification_hash")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    // Extract statistics
    let total_ips = data.get("legitimacy_analysis")
        .and_then(|v| v.get("total"))
        .and_then(|v| v.as_u64())
        .unwrap_or(0) as usize;
    
    let flagged_ips = data.get("legitimacy_analysis")
        .and_then(|v| v.get("flagged"))
        .and_then(|v| v.get("count"))
        .and_then(|v| v.as_u64())
        .unwrap_or(0) as usize;
    
    let country_count = data.get("geo_distribution")
        .and_then(|v| v.get("total_countries"))
        .and_then(|v| v.as_u64())
        .unwrap_or(0) as usize;
    
    let error_msg = if !hash_valid {
        Some(format!("Verification hash mismatch. Expected: {}, got: {}", verification_hash, calculated_hash))
    } else {
        None
    };
...
async fn verify_dsm_snapshot(data: &Value) -> Result<VerificationResult> {
    ...
    let _dsm_id = data.get("dsm_id")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    
    let _original_id = data.get("original_id")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    
    let _verification_nonce = data.get("verification_nonce")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    let entries_array = entries.as_array().ok_or_else(|| {
        SnapshotError::Validation("Entries section is not an array".to_string())
    })?;
    
    // Verify DSM hash
    let dsm_hash = data.get("dsm_verification_hash")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    let error_msg = if !hash_valid {
        Some(format!("DSM verification hash mismatch. Expected: {}, got: {}", dsm_hash, calculated_hash))
    } else {
        None
    };
...
async fn verify_attestation(data: &Value) -> Result<VerificationResult> {
    ...
    let _attestation_type = data.get("attestation_type")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    // Verify commitment
    let commitment = data.get("commitment")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    ...
    for snapshot in snapshots_array {
        let hash_hex = snapshot.get("hash")
            .and_then(|v| v.as_str())
            .unwrap_or_default();
        
        let hash_bytes = hex::decode(hash_hex).map_err(|e| {
            SnapshotError::Validation(format!("Failed to decode hash: {}", e))
        })?;
    ...
    let error_msg = if !commitment_valid {
        Some(format!("Commitment mismatch. Expected: {}, got: {}", commitment, calculated_commitment))
    } else {
        None
    };
