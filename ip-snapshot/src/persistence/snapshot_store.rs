...
impl SnapshotStore {
                    ...
                    match fs::read_to_string(&metadata_path).await {
                        Ok(content) => {
                            match serde_json::from_str::<SnapshotMetadata>(&content) {
                                ...
                                Err(e) => {
                                    error!("Failed to parse metadata for snapshot {}: {}", snapshot_id, e);
                                }
                            }
                        },
                        Err(e) => {
                            error!("Failed to read metadata for snapshot {}: {}", snapshot_id, e);
                        }
                    }
                }
            }
    ...
    async fn begin_transaction(&self, snapshot_id: &str) -> Result<SnapshotTransaction> {
        // Check if there's already a transaction in progress
        {
            ...
            if tx_guard.is_some() {
                return Err(SnapshotError::Database("Transaction already in progress".to_string()));
            }
    ...
    async fn commit_transaction(&self, transaction: &SnapshotTransaction) -> Result<()> {
        ...
        match transaction.state {
            ...
            TransactionState::Committed => {
                return Err(SnapshotError::Database("Transaction already committed".to_string()));
            },
            TransactionState::RolledBack => {
                return Err(SnapshotError::Database("Transaction already rolled back".to_string()));
            },
        }
        ...
        // Copy metadata
        fs::copy(&transaction.metadata_path, &target_metadata_path).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to copy metadata: {}", e))
        })?;
        
        // Copy data
        fs::copy(&transaction.data_path, &target_data_path).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to copy data: {}", e))
        })?;
    ...
    pub async fn save_snapshot(
        ...
        fs::write(&transaction.metadata_path, metadata_json).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to write metadata: {}", e))
        })?;
        
        // Write data
        let data_json = serde_json::to_string_pretty(&entries).map_err(|e| {
            SnapshotError::Serialization(e)
        })?;
        
        fs::write(&transaction.data_path, data_json).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to write data: {}", e))
        })?;
    ...
    /// Load a snapshot
    pub async fn load_snapshot(&self, snapshot_id: &str) -> Result<(Vec<IpEntry>, SnapshotMetadata)> {
        ...
        let metadata_str = fs::read_to_string(&metadata_path).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to read metadata: {}", e))
        })?;
        
        let metadata = serde_json::from_str::<SnapshotMetadata>(&metadata_str).map_err(|e| {
            SnapshotError::Serialization(e)
        })?;
        
        // Load data
        let data_path = snapshot_dir.join("data.json");
        let data_str = fs::read_to_string(&data_path).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to read data: {}", e))
        })?;
        
        let entries = serde_json::from_str::<Vec<IpEntry>>(&data_str).map_err(|e| {
            SnapshotError::Serialization(e)
        })?;
    ...
    pub fn get_snapshot_metadata(&self, snapshot_id: &str) -> Option<SnapshotMetadata> {
        self.metadata_cache.get(snapshot_id).map(|entry| entry.clone())
    }
    ...
    pub async fn delete_snapshot(&self, snapshot_id: &str) -> Result<()> {
        ...
        // Delete directory
        fs::remove_dir_all(&snapshot_dir).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to delete snapshot: {}", e))
        })?;
    ...
    pub async fn export_snapshot_json(&self, snapshot_id: &str, output_path: &Path) -> Result<()> {
        ...
        fs::write(output_path, json_str).await.map_err(|e| {
            SnapshotError::Database(format!("Failed to write export file: {}", e))
        })?;
    ...
    pub async fn get_snapshot_stats(&self, snapshot_id: &str) -> Result<serde_json::Value> {
        ...
        let top_countries: Vec<serde_json::Value> = country_stats.iter()
            .take(10)
            .map(|(code, count)| {
                serde_json::json!({
                    "country_code": code,
        ...
        // Get top 10 ASNs
        let top_asns: Vec<serde_json::Value> = asn_stats.iter()
            .take(10)
            .map(|(asn, count)| {
                serde_json::json!({
                    "asn": asn,
