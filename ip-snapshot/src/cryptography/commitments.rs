...
pub fn calculate_allocation_weight(
    ip_hash: &str,
...
pub fn generate_token_allocation(
    ...
    if ip_hashes.len() != commitment.total_ips {
        return Err(SnapshotError::Cryptographic(
            format!("IP count mismatch: {} hashes vs {} in commitment", 
                    ip_hashes.len(), commitment.total_ips)
        ));
    }
    ...
    if let Some((hash, _)) = weights.iter().max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap()) {
            *allocations.get_mut(hash).unwrap() += difference as u64;
            }
    },
    std::cmp::Ordering::Less => {
    // Remove tokens from the IP with lowest weight
    if let Some((hash, _)) = weights.iter().min_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap()) {
...
mod tests {
    ...
    fn test_token_allocation() {
        ...
        // Create IP hashes
        let ip_hashes: Vec<String> = entries.iter().map(|e| {
            let mut hasher = Hasher::new();
        ...
        let allocations2 = generate_token_allocation(&ip_hashes, &commitment, total_tokens).unwrap();
