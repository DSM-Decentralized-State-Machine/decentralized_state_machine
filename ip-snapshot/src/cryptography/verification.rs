                ...
                Ok(calculated_hash) => {
                    ...
                    if !valid {
                        self.add_error(format!(
                            ...
                            metadata.data_hash, calculated_hash.to_hex()
                        ));
                    }
                    valid
                },
...
pub fn generate_inclusion_proof(
    ...
    Ok(InclusionProof {
        ...
        entry_hash: entry.canonicalize()?.as_slice().hash(HashAlgorithm::Blake3)?,
        proof_indices,
        proof_hashes,
        root_hash,
    })
...
impl InclusionProof {
    ...
    pub fn verify(&self, entry: &IpEntry) -> Result<bool> {
        ...
        let calculated_entry_hash = entry.canonicalize()?.as_slice().hash(HashAlgorithm::Blake3)?;
