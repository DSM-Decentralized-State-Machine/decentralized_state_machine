...
impl HashDigest {
    ...
    pub fn from_hex(hex: &str, algorithm: HashAlgorithm) -> Result<Self> {
        let bytes = decode(hex)
            .map_err(|e| SnapshotError::Cryptographic(format!("Invalid hex string: {}", e)))?;
...
impl<T: Serialize> Hashable for T {
    fn hash(&self, algorithm: HashAlgorithm) -> Result<HashDigest> {
        ...
        let bytes = serde_json::to_vec(self)
            .map_err(|e| SnapshotError::Cryptographic(format!("Serialization failed: {}", e)))?;
        ...
        HashAlgorithm::Blake3 => {
            ...
            loop {
                let bytes_read = reader
                    .read(&mut buffer)
                    .map_err(|e| SnapshotError::Cryptographic(format!("Read error: {}", e)))?;
...
/// Hash a file
#[allow(dead_code)]
pub async fn hash_file(path: &std::path::Path, algorithm: HashAlgorithm) -> Result<HashDigest> {
    use tokio::fs::File;
    ...
    let mut file = File::open(path)
        .await
        .map_err(|e| SnapshotError::Cryptographic(format!("Failed to open file: {}", e)))?;

    // Read file content
    let mut content = Vec::new();
    file.read_to_end(&mut content)
        .await
        .map_err(|e| SnapshotError::Cryptographic(format!("Failed to read file: {}", e)))?;
...
#[allow(dead_code)]
pub fn combine_hashes(hashes: &[HashDigest], algorithm: HashAlgorithm) -> Result<HashDigest> {
    if hashes.is_empty() {
    ...
    let normalized_hashes: Vec<HashDigest> =
        hashes.iter().map(|h| h.to_algorithm(algorithm)).collect();
...
mod tests {
    ...
    fn test_combine_hashes() {
        ...
        let combined = combine_hashes(
            &[hash1.clone(), hash2.clone(), hash3.clone()],
            HashAlgorithm::Blake3,
        )
        .unwrap();
