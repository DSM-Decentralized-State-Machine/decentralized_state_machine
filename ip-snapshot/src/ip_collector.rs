...
impl IpCollector {
    ...
    pub async fn add_ip(&self, ip: IpAddr) -> Result<()> {
        ...
        if !*self.state.is_collecting.read() {
            return Err(SnapshotError::Validation(
                "Collection is not active".to_string(),
            ));
        }
        ...
        if let Some(max_ips) = self.config.max_ips {
            if self.state.ip_entries.len() >= max_ips {
                return Err(SnapshotError::Validation(
                    "Maximum IP limit reached".to_string(),
                ));
            }
        }
            ...
            match geo_service.lookup(ip).await {
                Ok(geo_info) => {
                    ...
                    if let Some(country_code) =
                        &entry.geo.as_ref().and_then(|g| g.country_code.clone())
                    {
    ...
    pub async fn start_collection(&self) -> Result<()> {
        ...
        if *self.state.is_collecting.read() {
            return Err(SnapshotError::Validation(
                "Collection is already active".to_string(),
            ));
        }
    ...
    pub async fn stop_collection(&self) -> Result<()> {
        ...
        *self.state.is_collecting.write() = false;
        ...
        // Calculate data hash first before updating metadata
    ...
    pub async fn create_snapshot(&self) -> Result<String> {
        ...
        if self.state.ip_entries.is_empty() {
            return Err(SnapshotError::Validation(
                "No IP addresses collected".to_string(),
            ));
        }
        ...
        // Store snapshot
        self.store
            .save_snapshot(&snapshot_id, entries, metadata)
            .await?;

        info!(
            "Created snapshot {} with {} IPs",
            snapshot_id,
            self.state.ip_entries.len()
        );
    ...
    pub async fn get_stats(&self) -> Result<CollectionStats> {
        ...
        let mut countries: Vec<CountryStats> = country_map
            .iter()
            .map(|(code, (name, count, flagged))| CountryStats {
    ...
    pub async fn clear(&self) -> Result<()> {
        ...
        if *self.state.is_collecting.read() {
            return Err(SnapshotError::Validation(
                "Cannot clear data while collection is active".to_string(),
            ));
        }
    ...
    pub async fn run(mut self) {
        ...
        while let Some(cmd) = self.command_rx.recv().await {
            match cmd {
                ...
                CollectorCommand::CreateSnapshot => match self.create_snapshot().await {
