...
impl SnapshotConfig {
    ...
    pub async fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let config: Self = serde_json::from_str(&file_content).map_err(|e| {
            SnapshotError::Configuration(format!("Failed to parse config: {}", e))
        })?;
                ...
                if let Some(log_dir) = log_path.parent() {
                    if !log_dir.exists() {
                        fs::create_dir_all(log_dir).await.map_err(|e| {
                            SnapshotError::Configuration(format!("Failed to create log directory: {}", e))
                        })?;
                    }
                }
            }
