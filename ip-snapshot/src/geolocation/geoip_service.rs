    ...
    pub async fn lookup(&self, ip: IpAddr) -> Result<GeoInformation> {
        ...
        let city_result = match self.reader.lookup::<geoip2::City>(ip) {
            ...
            Err(e) => {
                ...
                return Err(SnapshotError::Geolocation(
                    format!("GeoIP lookup failed: {}", e)
                ));
            }
        };
        
        // Extract location information
        let coordinates = city_result.location.as_ref().and_then(|loc| {
        ...
        // Extract city name
        let city = city_result.city
            .and_then(|c| c.names)
            .and_then(|n| n.get("en").map(|s| s.to_string()));
        
        // Extract continent code
        let continent_code = city_result.continent
            .and_then(|c| c.code)
            .map(|s| s.to_string());
        
        // Extract time zone
        let time_zone = city_result.location
