...
impl FraudDetector {
    ...
    fn populate_default_vpn_ranges(&self) {
        ...
        self.vpn_subnets.insert("5.253.206.0/24".to_string(), ProxyType::Vpn);
        self.vpn_subnets.insert("31.13.191.0/24".to_string(), ProxyType::Vpn);
        self.vpn_subnets.insert("45.134.140.0/24".to_string(), ProxyType::Vpn);
        
        // ExpressVPN ranges (sample)
        self.vpn_subnets.insert("108.177.236.0/24".to_string(), ProxyType::Vpn);
        self.vpn_subnets.insert("185.254.75.0/24".to_string(), ProxyType::Vpn);
        
        // Tor exit nodes (sample)
        self.vpn_subnets.insert("51.15.43.0/24".to_string(), ProxyType::Tor);
        self.vpn_subnets.insert("185.220.101.0/24".to_string(), ProxyType::Tor);
        
        // Public proxies (sample)
        self.vpn_subnets.insert("104.149.129.0/24".to_string(), ProxyType::PublicProxy);
        self.vpn_subnets.insert("167.71.0.0/16".to_string(), ProxyType::DataCenter);
        
        // Common datacenter ranges
        self.datacenter_ranges.insert("35.184.0.0/13".to_string(), "Google Cloud".to_string());
        self.datacenter_ranges.insert("13.64.0.0/11".to_string(), "Microsoft Azure".to_string());
        self.datacenter_ranges.insert("52.0.0.0/8".to_string(), "Amazon AWS".to_string());
    ...
    fn populate_asn_reputation(&self) {
        ...
        info!("Populated ASN reputation database with {} entries", self.asn_reputation.len());
    }
    ...
    pub async fn check_ip(&self, ip: IpAddr) -> Result<u8> {
        ...
        for item in self.vpn_subnets.iter() {
            ...
            if subnet.starts_with(&ip_prefix) {
                ...
                debug!("IP {} matched VPN range {}, reducing score to {}", ip, subnet, score);
                break;
            }
        }
        ...
        for item in self.datacenter_ranges.iter() {
            ...
            if subnet.starts_with(&ip_prefix) {
                ...
                debug!("IP {} matched datacenter range {} ({}), reducing score to {}", 
